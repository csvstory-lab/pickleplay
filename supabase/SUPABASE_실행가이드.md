...
## 7단계 — 알림 시스템 (선택 · 유저 앱 알림함)

아래 **2개 SQL**을 **위에서부터 순서대로** 실행합니다.

| 순서 | 파일 | 내용 |
|:---:|------|------|
| A | `23_create_notifications_table.sql` | `notifications` 테이블 + RLS |
| B | `24_notification_triggers.sql` | 댓글 트리거 + 마감/명예 pg_cron |

### 실행 후 Dashboard 설정 (2가지)

1. **Extensions** → `pg_cron` **활성화**  
   - 마감(`end`)·전당(`honor`) 알림이 **1분마다** 자동 처리됩니다.  
   - **최대 1분 지연**은 P!CKLE 서비스 정책상 허용된 UX입니다.

2. **Replication** → `notifications` 테이블 **Realtime ON**  
   - 새 알림이 생기면 앱 종 아이콘 빨간 점이 **새로고침 없이** 갱신됩니다.

### 알림 종류 요약

| type | 발송 시점 | 수신자 |
|------|-----------|--------|
| `comment` | 댓글 작성 즉시 | 불판 작성자 (본인 댓글 제외) |
| `end` | 마감 후 cron (≤1분) | 투표 참여자 + 작성자 |
| `honor` | 마감 후 cron (≤1분) | 전당 후보 조건 충족 시 작성자 |

### 수동 테스트 (SQL Editor)

```sql
-- 마감/명예 알림 즉시 처리 (cron 대기 없이)
SELECT public.pickle_process_due_post_notifications();
```

**확인:** Table Editor → `notifications` · 유저 앱 → 🔔 → `notifications.html`
