/**
 * P!CKLE — 카카오톡/SNS 공유용 OG 리다이렉트
 *
 * GET /functions/v1/share-redirect?id=<postId>
 *
 * 크롤러 봇(카카오톡, 페이스북, 트위터 등)은 JS를 실행하지 않으므로
 * <head>의 OG/Twitter 메타 태그만 읽어 미리보기 카드를 만들고,
 * 실제 유저의 브라우저는 <body>의 스크립트가 즉시 실행되어
 * 불판 상세 페이지로 리다이렉트된다. 하나의 HTML 응답으로 양쪽을 모두 처리한다.
 */

const FALLBACK_SUPABASE_URL = 'https://jszgznanptutwxcsnrep.supabase.co';
const SITE_ORIGIN = 'https://pickleplay.kr';

function getSupabaseUrl(): string {
  const fromEnv = String(Deno.env.get('SUPABASE_URL') || '').trim();
  return (fromEnv || FALLBACK_SUPABASE_URL).replace(/\/+$/, '');
}
...
Deno.serve((req) => {
  ...
  const ogImageUrl = `${getSupabaseUrl()}/functions/v1/generate-og?postId=${encodeURIComponent(id)}`;
  const detailUrl = id
    ? `${SITE_ORIGIN}/user_app/detail.html?id=${encodeURIComponent(id)}`
